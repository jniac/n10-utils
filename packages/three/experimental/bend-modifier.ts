...
export function getBendModifier(matrix: Matrix4) {
  ...
  return {
    ...
    applyBendX: (radius: number, point: Vector3, out = point): Vector3 => {
      if (radius > 1e12) {
        return out.copy(point)
      }
      const { x, y, z } = _local.copy(point).applyMatrix4(_im)
      const a = x / (radius - z)
      const ox = (radius - z) * Math.sin(a)
      const oz = radius - (radius - z) * Math.cos(a)
      return out
        .set(ox, y, oz)
        .applyMatrix4(_m)
    },

    applyBendY: (radius: number, point: Vector3, out = point): Vector3 => {
      if (radius > 1e12) {
        return out.copy(point)
      }
      const { x, y, z } = _local.copy(point).applyMatrix4(_im)
      const a = y / (radius - z)
      const oy = (radius - z) * Math.sin(a)
      const oz = radius - (radius - z) * Math.cos(a)
      return out
        .set(x, oy, oz)
        .applyMatrix4(_m)
